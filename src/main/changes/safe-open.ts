...
export type OpenTarget =
  /** The file or folder, which really lies inside the workspace. */
  | { exists: true; path: string }
  /** Nothing is there; `path` is the nearest existing folder inside the workspace. */
...
export async function resolveOpenTarget(workspacePath: string, path: string): Promise<OpenTarget> {
  ...
  for (;;) {
    try {
      ...
      return { exists: candidate === lexical, path: candidate };
    } catch (error) {
      if (!isMissing(error)) throw error;
    }
    const parent = dirname(candidate);
    if (candidate === base || parent === candidate || !isWithin(base, parent)) {
      return { exists: false, path: base };
    }
    candidate = parent;
  }
}
