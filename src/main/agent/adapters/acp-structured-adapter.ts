...
export interface AcpStructuredTransportFactoryOptions {
...
export type AcpStructuredTransportFactory = (
  options: AcpStructuredTransportFactoryOptions
) => Promise<LineJsonRpcTransport>;

export interface CreateAcpStructuredAdapterOptions {
  createTransport?: AcpStructuredTransportFactory;
  resolveAuthenticationMethod?: () => Promise<string | null>;
}

export type GeminiStructuredTransportFactoryOptions =
  AcpStructuredTransportFactoryOptions;
export type GeminiStructuredTransportFactory = AcpStructuredTransportFactory;
export type CreateGeminiStructuredAdapterOptions = CreateAcpStructuredAdapterOptions;
...
export function createAcpStructuredAdapter(
  context: StructuredAgentAdapterContext,
  profile: AcpProviderProfile,
  options: CreateAcpStructuredAdapterOptions = {}
): StructuredAgentAdapter {
  if (context.providerId !== profile.providerId) {
    throw new Error(`The ${profile.displayName} ACP adapter requires a ${profile.displayName} context.`);
  }
  ...
  const providerName = profile.displayName;
  const providerKey = profile.providerId;
  ...
  const ensureSession = (params: Record<string, unknown>): void => {
    if (params.sessionId !== nativeSessionId) {
      throw new Error(`The ${providerName} request belongs to another session.`);
    }
  };

  const handleRequest = async (request: JsonRpcProviderRequest): Promise<unknown> => {
    const params = object(request.params);
    if (params === null) throw new Error(`The ${providerName} client request is invalid.`);
    ensureSession(params);
    if (request.method === 'fs/read_text_file') {
      if (typeof params.path !== 'string') {
        throw new Error(`The ${providerName} file path is invalid.`);
      }
      ...
      if (Buffer.byteLength(content) > 1024 * 1024) {
        throw new Error(`The ${providerName} file exceeds Lumora’s read limit.`);
      }
    ...
    if (request.method === 'fs/write_text_file') {
      if (typeof params.path !== 'string' || typeof params.content !== 'string') {
        throw new Error(`The ${providerName} write request is invalid.`);
      }
      if (Buffer.byteLength(params.content) > 1024 * 1024) {
        throw new Error(`The ${providerName} write exceeds Lumora’s safety limit.`);
      }
      const path = await containedPath(
        context.launch.workingDirectory,
        params.path,
    ...
    if (request.method !== 'session/request_permission') {
      throw new Error(`The ${providerName} client method is not supported.`);
    }
    ...
    if (toolCallId === null || choices.length === 0) {
      throw new Error(`The ${providerName} permission request is invalid.`);
    }
    const approvalId = `${providerKey}-${toolCallId}`.slice(0, 256);
    const turnId = currentTurnId ?? `${providerKey}-turn-${Math.max(1, turnNumber)}`;
    deliver({
      turnId,
      parentEventId: null,
      ...
      payload: {
        approvalId,
        title: bounded(
          typeof toolCall?.title === 'string' ? toolCall.title : `${providerName} tool request`,
          512
        ),
        detail: `${providerName} needs permission to continue this operation.`,
  ...
  const acceptNotification = (notification: JsonRpcNotification): void => {
    ...
    const turnId = currentTurnId ?? `${providerKey}-history-${initialEvents.length + 1}`;
  ...
  const runPrompt = async (text: string, attachmentTokens: readonly string[]): Promise<void> => {
    ...
    try {
      ...
      if (usage.success) {
        deliver({
          ...
          payload: {
            ...
            cachedInputTokens: usage.data.cachedReadTokens ?? null,
      ...
      deliver({
        ...
        parentEventId: null,
        kind: 'turn.completed',
        payload: { state: 'failed', message: `${providerName} could not complete this turn.` }
      });
    } finally {
  ...
  return {
    async open() {
      ...
      if (closed) {
        ...
        throw new Error(`The ${providerName} structured session was closed during startup.`);
      }
      ...
      if (
        profile.authentication === 'gemini_configured' &&
        authenticationMethod === null
      ) {
        throw new Error(`${providerName} has no configured authentication method.`);
      }
      if (authenticationMethod !== null) {
        if (!initialized.authMethods.some(({ id }) => id === authenticationMethod)) {
          throw new Error(`${providerName} does not support the configured authentication method.`);
        }
      ...
      if (context.launch.request.strategy === 'new') {
        ...
        if (sessionResponse.sessionId === undefined) {
          throw new Error(`${providerName} did not return a native session id.`);
        }
        nativeSessionId = sessionResponse.sessionId;
      } else {
        if (!initialized.agentCapabilities.loadSession || nativeSessionId === null) {
          throw new Error(`${providerName} cannot load this session.`);
        }
    ...
    async dispatch(action: StructuredAgentAction) {
      ...
      if (action.kind === 'command.execute') {
        ...
        if (command.id === 'model') {
          ...
          if (model === undefined) {
            throw new Error(`The ${providerName} model is not available.`);
          }
          if (modelConfigId !== null) {
            ...
            throw new Error(`${providerName} model selection is unavailable.`);
          }
          return;
        }
        void runPrompt(
...
export function createGeminiStructuredAdapter(
  ...
  return createAcpStructuredAdapter(
    context,
    acpProviderProfile('gemini'),
    {
      ...options,
      resolveAuthenticationMethod: options.resolveAuthenticationMethod
